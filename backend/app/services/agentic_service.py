...
class EnhancedAgenticATSService:
    ...
    def _create_validator(self) -> Agent:
        return Agent(
            role="Skill Verification Specialist",
            goal="Verify that all extracted skills are legitimately present in the source text",
            backstory="""You are a skeptical auditor who ensures accuracy and prevents
            hallucinations in AI-extracted data. You check if the mentioned skill actually exists in the provided context.""",
            llm=self.llm,
            verbose=False,
